        ...
        <div class="container">
            <div class="row">
                <div class="col-lg-8 mx-auto text-center">
                    ...
                    @endif
                    <p class="mt-2 mb-0">
                        <small>{{ $posts->total() }} {{ Str::plural('post', $posts->total()) }} in this category</small>
                    </p>
                </div>
            </div>
        </div>
            ...
            <div class="row g-4">
                ...
                <div class="col-lg-4">
                    ...
                    @if(isset($categories) && $categories->count() > 0)
                        <div class="card border-0 shadow-sm mb-4">
                            <div class="card-body">
                                <h5 class="card-title fw-bold mb-3">All Categories</h5>
                                <ul class="list-unstyled mb-0">
                                    @foreach($categories as $cat)
                                        <li class="mb-2">
                                            <a href="{{ route('blog.category', $cat->slug) }}" class="text-decoration-none d-flex justify-content-between align-items-center {{ $category->id == $cat->id ? 'fw-bold text-primary' : '' }}">
                                                <span>
                                                    <i class="fas fa-folder me-2 {{ $category->id == $cat->id ? 'text-primary' : 'text-muted' }}"></i>{{ $cat->name }}
                                                </span>
                                                <span class="badge {{ $category->id == $cat->id ? 'bg-primary' : 'bg-light text-dark' }}">
                                                    {{ $cat->posts_count ?? 0 }}
                                                </span>
                                            </a>
                                        </li>
                    ...
                    @if($popularPosts->count() > 0)
                        <div class="card border-0 shadow-sm mb-4">
                            <div class="card-body">
                                <h5 class="card-title fw-bold mb-3">Popular in {{ $category->name }}</h5>
                                <div class="list-group list-group-flush">
                                    @foreach($popularPosts as $popularPost)
                                        <a href="{{ route('blog.show', $popularPost->slug) }}" class="list-group-item list-group-item-action border-0 px-0">
                                            <div class="d-flex">
                                                @if($popularPost->featured_image)
                                                    <img src="{{ asset('storage/' . $popularPost->featured_image) }}" class="me-3 rounded" alt="{{ $popularPost->title }}" style="width: 60px; height: 60px; object-fit: cover;">
                                                @endif
                                                <div>
                                                    <h6 class="mb-1 small">{{ Str::limit($popularPost->title, 50) }}</h6>
                                                    <small class="text-muted">
                                                        <i class="far fa-eye me-1"></i>{{ $popularPost->views_count ?? 0 }} views
                                                    </small>
                                                </div>
                                            </div>
                                        </a>
